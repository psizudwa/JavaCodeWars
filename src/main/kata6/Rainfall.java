...
public class Rainfall {
    public static double mean(String town, String strng) {
        List<Double> dataDouble = filter(strng,town);
        if(dataDouble.isEmpty()){
            return -1;
        }else {
            return dataDouble.stream().mapToDouble(d -> d).sum()/dataDouble.size();
        }
    }
    public static double variance(String town, String strng) {
        ...
        if(dataDouble.isEmpty()){
            ...
            return dataDouble.stream().mapToDouble(x ->Math.pow(x-mean,2)).sum()/(dataDouble.size());
        }
    }
    public static List<Double> filter(String data, String town){
        String[] dataArray = data.split("\n");
        List<Double> dataDouble = new ArrayList<Double>(12);
        for(String subStr : dataArray){
            if (subStr.split(":")[0].equals(town)){
                for(String subSubStr : subStr.split(":")[1].split(",")){
                    dataDouble.add(Double.parseDouble(subSubStr.split(" ")[1]));
                }
            }
        }
        return dataDouble;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Rainfall.variance("Beijing", data));
    }
}
